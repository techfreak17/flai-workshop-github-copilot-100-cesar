    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? details.participants.map(p => `<li data-email="${p}"><span>${p}</span><button class="delete-participant" title="Remove participant">×</button></li>`).join('')
        ...
        deleteButtons.forEach((button) => {
          button.addEventListener("click", async (e) => {
            e.preventDefault();
            const email = e.target.closest("li").getAttribute("data-email");
